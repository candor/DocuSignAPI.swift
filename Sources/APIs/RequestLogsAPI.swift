...
open class RequestLogsAPI {
    ...
    open class func aPIRequestLogGetRequestLogSettings(headers: HTTPHeaders = DocuSignAPI.customHeaders, beforeSend: (inout ClientRequest) throws -> Void = { _ in }) -> EventLoopFuture<APIRequestLogGetRequestLogSettings> {
        ...
        return apiClient.send(.GET, headers: headers, to: URI(string: URLString)) { request in
            ...
            switch response.status.code {
            case 200:
                return .http200(value: try? response.content.decode(DiagnosticsSettingsInformation.self), raw: response)
            case 400:
                return .http400(value: try? response.content.decode(ErrorDetails.self), raw: response)
            default:
                return .http0(value: try? response.content.decode(DiagnosticsSettingsInformation.self), raw: response)
            }
        }
    }
    ...
    open class func aPIRequestLogGetRequestLogs(encoding: String? = nil, headers: HTTPHeaders = DocuSignAPI.customHeaders, beforeSend: (inout ClientRequest) throws -> Void = { _ in }) -> EventLoopFuture<APIRequestLogGetRequestLogs> {
        ...
        return apiClient.send(.GET, headers: headers, to: URI(string: URLString)) { request in
            ...
            switch response.status.code {
            case 200:
                return .http200(value: try? response.content.decode(ApiRequestLogsResult.self), raw: response)
            case 400:
                return .http400(value: try? response.content.decode(ErrorDetails.self), raw: response)
            default:
                return .http0(value: try? response.content.decode(ApiRequestLogsResult.self), raw: response)
            }
        }
    }
