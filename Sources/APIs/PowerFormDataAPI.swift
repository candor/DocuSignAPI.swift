...
open class PowerFormDataAPI {
    ...
    open class func powerFormsGetPowerFormFormData(accountId: String, powerFormId: String, dataLayout: String? = nil, fromDate: String? = nil, toDate: String? = nil, headers: HTTPHeaders = DocuSignAPI.customHeaders, beforeSend: (inout ClientRequest) throws -> Void = { _ in }) -> EventLoopFuture<PowerFormsGetPowerFormFormData> {
        ...
        }.flatMapThrowing { response -> PowerFormsGetPowerFormFormData in
            switch response.status.code {
            case 200:
                return .http200(value: try? response.content.decode(PowerFormsFormDataResponse.self), raw: response)
            case 400:
                return .http400(value: try? response.content.decode(ErrorDetails.self), raw: response)
            default:
                return .http0(value: try? response.content.decode(PowerFormsFormDataResponse.self), raw: response)
            }
        }
    }
}
