...
open class ENoteConfigurationsAPI {
    ...
    open class func eNoteConfigurationGetENoteConfiguration(accountId: String, headers: HTTPHeaders = DocuSignAPI.customHeaders, beforeSend: (inout ClientRequest) throws -> Void = { _ in }) -> EventLoopFuture<ENoteConfigurationGetENoteConfiguration> {
        ...
        return apiClient.send(.GET, headers: headers, to: URI(string: URLString)) { request in
            ...
            switch response.status.code {
            case 200:
                return .http200(value: try? response.content.decode(ENoteConfiguration.self), raw: response)
            case 400:
                return .http400(value: try? response.content.decode(ErrorDetails.self), raw: response)
            default:
                return .http0(value: try? response.content.decode(ENoteConfiguration.self), raw: response)
            }
        }
    }
