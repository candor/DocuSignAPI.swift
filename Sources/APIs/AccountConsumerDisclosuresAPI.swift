...
open class AccountConsumerDisclosuresAPI {
    ...
    open class func consumerDisclosureGetConsumerDisclosure(accountId: String, langCode: String? = nil, headers: HTTPHeaders = DocuSignAPI.customHeaders, beforeSend: (inout ClientRequest) throws -> Void = { _ in }) -> EventLoopFuture<ConsumerDisclosureGetConsumerDisclosure> {
        ...
        return apiClient.send(.GET, headers: headers, to: URI(string: URLString)) { request in
            ...
            case 200:
                return .http200(value: try? response.content.decode(AccountConsumerDisclosures.self), raw: response)
            case 400:
                return .http400(value: try? response.content.decode(ErrorDetails.self), raw: response)
            default:
                return .http0(value: try? response.content.decode(AccountConsumerDisclosures.self), raw: response)
            }
        }
    }
