//
// PowerFormSendersResponse.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation
import Vapor

/** This object includes information about the users who have sent PowerForms. */
public final class PowerFormSendersResponse: Content {
    /** The last index position in the result set.  */
    public var endPosition: Int?
    /** The URI for the next chunk of records based on the search request. It is `null` if this is the last set of results for the search.  */
    public var nextUri: String?
    /** An array of `userInfo` objects containing information about users who have sent PowerForms. */
    public var powerFormSenders: [UserInfo]?
    /** The URI for the prior chunk of records based on the search request. It is `null` if this is the first set of results for the search.  */
    public var previousUri: String?
    /** The number of results in this response. Because you can filter which entries are included in the response, this value is always less than or equal to the `totalSetSize`. */
    public var resultSetSize: Int?
    /** The starting index position of the current result set. */
    public var startPosition: Int?
    /** The total number of items in the result set. This value is always greater than or equal to the value of `resultSetSize`. */
    public var totalSetSize: Int?

    public init(endPosition: Int? = nil, nextUri: String? = nil, powerFormSenders: [UserInfo]? = nil, previousUri: String? = nil, resultSetSize: Int? = nil, startPosition: Int? = nil, totalSetSize: Int? = nil) {
        self.endPosition = endPosition
        self.nextUri = nextUri
        self.powerFormSenders = powerFormSenders
        self.previousUri = previousUri
        self.resultSetSize = resultSetSize
        self.startPosition = startPosition
        self.totalSetSize = totalSetSize
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case endPosition
        case nextUri
        case powerFormSenders
        case previousUri
        case resultSetSize
        case startPosition
        case totalSetSize
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)

        try container.encodeIfPresent(endPosition, forKey: .endPosition)
        try container.encodeIfPresent(nextUri, forKey: .nextUri)
        try container.encodeIfPresent(powerFormSenders, forKey: .powerFormSenders)
        try container.encodeIfPresent(previousUri, forKey: .previousUri)
        try container.encodeIfPresent(resultSetSize, forKey: .resultSetSize)
        try container.encodeIfPresent(startPosition, forKey: .startPosition)
        try container.encodeIfPresent(totalSetSize, forKey: .totalSetSize)
    }

    // Decodable protocol methods

    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)

        endPosition = try container.decodeIfPresent(Int.self, forKey: .endPosition)
        nextUri = try container.decodeIfPresent(String.self, forKey: .nextUri)
        powerFormSenders = try container.decodeIfPresent([UserInfo].self, forKey: .powerFormSenders)
        previousUri = try container.decodeIfPresent(String.self, forKey: .previousUri)
        resultSetSize = try container.decodeIfPresent(Int.self, forKey: .resultSetSize)
        startPosition = try container.decodeIfPresent(Int.self, forKey: .startPosition)
        totalSetSize = try container.decodeIfPresent(Int.self, forKey: .totalSetSize)
    }
}

extension PowerFormSendersResponse: Hashable {
    public static func == (lhs: PowerFormSendersResponse, rhs: PowerFormSendersResponse) -> Bool {
        lhs.endPosition == rhs.endPosition &&
            lhs.nextUri == rhs.nextUri &&
            lhs.powerFormSenders == rhs.powerFormSenders &&
            lhs.previousUri == rhs.previousUri &&
            lhs.resultSetSize == rhs.resultSetSize &&
            lhs.startPosition == rhs.startPosition &&
            lhs.totalSetSize == rhs.totalSetSize
    }

    public func hash(into hasher: inout Hasher) {
        hasher.combine(endPosition?.hashValue)
        hasher.combine(nextUri?.hashValue)
        hasher.combine(powerFormSenders?.hashValue)
        hasher.combine(previousUri?.hashValue)
        hasher.combine(resultSetSize?.hashValue)
        hasher.combine(startPosition?.hashValue)
        hasher.combine(totalSetSize?.hashValue)
    }
}
