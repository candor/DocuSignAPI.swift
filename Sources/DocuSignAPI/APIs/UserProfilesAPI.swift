...
open class UserProfilesAPI {
     ...
     */
    open class func userProfileGetProfileRaw(accountId: String, userId: String, headers: HTTPHeaders = DocuSignAPI.customHeaders, beforeSend: (inout ClientRequest) throws -> Void = { _ in }) -> EventLoopFuture<ClientResponse> {
    ...
    open class func userProfileGetProfile(accountId: String, userId: String, headers: HTTPHeaders = DocuSignAPI.customHeaders, beforeSend: (inout ClientRequest) throws -> Void = { _ in }) -> EventLoopFuture<UserProfileGetProfile> {
        return userProfileGetProfileRaw(accountId: accountId, userId: userId, headers: headers, beforeSend: beforeSend).flatMapThrowing { response -> UserProfileGetProfile in
    ...
    /**
     ...
     - returns: `EventLoopFuture` of `ClientResponse`
     */
    open class func userProfilePutProfileRaw(accountId: String, userId: String, userProfile: UserProfile? = nil, headers: HTTPHeaders = DocuSignAPI.customHeaders, beforeSend: (inout ClientRequest) throws -> Void = { _ in }) -> EventLoopFuture<ClientResponse> {
        var path = "/v2.1/accounts/{accountId}/users/{userId}/profile"
        let accountIdPreEscape = String(describing: accountId)
        let accountIdPostEscape = accountIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{accountId}", with: accountIdPostEscape, options: .literal, range: nil)
    ...
    open class func userProfilePutProfile(accountId: String, userId: String, userProfile: UserProfile? = nil, headers: HTTPHeaders = DocuSignAPI.customHeaders, beforeSend: (inout ClientRequest) throws -> Void = { _ in }) -> EventLoopFuture<UserProfilePutProfile> {
        return userProfilePutProfileRaw(accountId: accountId, userId: userId, userProfile: userProfile, headers: headers, beforeSend: beforeSend).flatMapThrowing { response -> UserProfilePutProfile in
            switch response.status.code {
            case 200:
