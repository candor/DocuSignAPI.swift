...
let accountId = "accountId_example" // String | The external account number (int) or account ID GUID.
let bccEmailArchive = bccEmailArchive(accountId: "accountId_example", bccEmailArchiveId: "bccEmailArchiveId_example", created: "created_example", createdBy: nil, email: "email_example", emailNotificationId: "emailNotificationId_example", modified: "modified_example", modifiedBy: nil, status: "status_example", uri: "uri_example") // BccEmailArchive | Boolean that specifies whether BCC for Email Archive is enabled for the account. BCC for Email Archive allows you to set up an archive email address so that a BCC copy of an envelope is sent only to that address. (optional)
